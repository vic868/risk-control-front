  ...
  <div class="page-container">
    <a-form :model="form" layout="vertical">
      <a-form-item label="策略名称" :rules="[{ required: true, message: '请输入策略名称' }]">
        <a-input v-model:value="form.name" placeholder="请输入策略名称" :disabled="true" />
      </a-form-item>
      
      <a-form-item label="策略描述">
        <a-textarea v-model:value="form.description" placeholder="请输入策略描述" :rows="3" />
      </a-form-item>
      
      <a-form-item label="策略类型">
        <a-select v-model:value="form.type" :disabled="true">
...
const loadData = async () => {
  ...
  try {
    ...
    const data = await strategyApi.query({ name })
    if (data) {
      form.name = data.name
